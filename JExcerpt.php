...
class J_Excerpt {
  ...
  public function __construct( $post, $more_link_options = null, $excerpt_length = null, $echo = false ) {
    if( is_numeric( $post ) ) $post = get_post( $post );
    $this->post = $post;

    if( ! is_null( $more_link_options ) && ! empty( $more_link_options ) ) {
      foreach( $more_link_options as $ml_option_key => $ml_option_value ) {
        if( property_exists( 'JExcerpt', 'more_link_' . $ml_option_key ) ) $this->{'more_link_' . $ml_option_key} = $ml_option_value;
      }
    }
  ...
  private function get_excerpt_text( $id ) {
            ...
            if ( ! $excerpt = trim( $post->post_excerpt ) ) {
              ...
              if( $more_tag_location = strpos( $excerpt, "<!--more-->" ) ) {
                ...
                $words = preg_split("/[\n\r\t ]+/", $excerpt, $this->excerpt_length + 1, PREG_SPLIT_NO_EMPTY);
